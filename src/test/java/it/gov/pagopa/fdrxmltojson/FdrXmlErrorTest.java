...
class FdrXmlErrorTest {
  ...
  @InjectMocks private FdrXmlError fdrXmlError;
  ...
  void setUp() throws ApiException {
    ...
    lenient()
        .when(mockTableClient.listEntities(any(), any(), any()))
        .thenReturn(TestUtil.createPagedIterable(new IterableStream<>(entities)));
  ...
  void testRun_withValidPartitionAndRowKey() {
    when(request.getQueryParameters())
        .thenReturn(Map.of("partitionKey", "testPartition", "rowKey", "testRow"));
  ...
  void testRunErrorRecovery_withValidPartitionAndRowKeys() {
    when(request.getBody())
        .thenReturn(
            Optional.of(
                "{\"partitionKey\":\"2025-03-26\",\"rowKeys\":[\"0f9aa641-5cd7-474d-a40b-9b815aa02629\",\"1652bccb-37be-4c25-94bb-45cbc711be56\"]}"));
  ...
  void testRunErrorRecovery_withoutPartition() {
    when(request.getBody())
        .thenReturn(
            Optional.of(
                "{\"partitionKey\":\"\",\"rowKeys\":[\"0f9aa641-5cd7-474d-a40b-9b815aa02629\",\"1652bccb-37be-4c25-94bb-45cbc711be56\"]}"));
    HttpResponseMessage response = fdrXmlError.runErrorRecovery(request, context);
  ...
  void testRunErrorRecovery_withValidPartitionAndNoRowKeys() {
    when(request.getBody())
        .thenReturn(Optional.of("{\"partitionKey\":\"2025-03-26\",\"rowKeys\":[]}"));
    HttpResponseMessage response = fdrXmlError.runErrorRecovery(request, context);
